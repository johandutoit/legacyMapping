...
OpenLayers.Format.WMC.vSAEON = OpenLayers.Class(OpenLayers.Format.XML, {
    ...
    */
    createLayerWMS: function(layerInfo) {
        ...
        var style = '';
        if (layerInfo.styles && layerInfo.styles.length && layerInfo.styles[0].name)
            style = layerInfo.styles[0].name;
        
        return CreateWMS(layerInfo.title, layerInfo.href, layerInfo.params.layers, options.transparent, options.visibility, layerInfo.options.opacity, "", "", "", style);
    },
